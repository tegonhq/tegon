...
export default class WorkflowsService {
  ...
  async getAllWorkflows(teamRequestIdBody: TeamRequestIdBody,): Promise<Workflow[]> {
  ...
  async deleteWorkflow(workflowRequestIdBody: WorkflowRequestIdBody): Promise<Workflow> {
    return await this.prisma.workflow.delete({
      where: {
        id:
        workflowRequestIdBody.workflowId,
      },
    });
  }
}
