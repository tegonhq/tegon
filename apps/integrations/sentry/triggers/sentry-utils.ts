...
export async function getSentryHeaders(
  ...
  const sentryAccessToken = await getAccessToken(
    integrationAccount,
    accesstoken
  );
  return {
...
export async function getSentryIssue(
  ...
  return await getRequest(
    url,
    await getSentryHeaders(integrationAccount, accesstoken)
  );
}
...
export async function createSentryComment(
  ...
  const headers = await getSentryHeaders(integrationAccount, accesstoken);
